function finedining(){
	document.getElementById("bodyeffects").style.backgroundImage = "url('images/plates/finedining.png')";

	document.getElementById("picnicbtn").style.backgroundColor = "white";
	document.getElementById("picnicbtn").style.border = "#13448d solid 3px";
	document.getElementById("picnicbtn").style.color = "#13448d";
	document.getElementById("diningroombtn").style.backgroundColor = "white";
	document.getElementById("diningroombtn").style.border = "#13448d solid 3px";
	document.getElementById("diningroombtn").style.color = "#13448d";
	document.getElementById("finediningbtn").style.backgroundColor = "#13448d";
	document.getElementById("finediningbtn").style.border = "#13448d solid 3px";
	document.getElementById("finediningbtn").style.color = "white";


	document.getElementById("plate1").style.backgroundImage = "url('images/plates/plate2.png')";
	document.getElementById("plate1").style.backgroundSize = "contain";
	document.getElementById("plate1").style.width = "796px";
	document.getElementById("plate1").style.height = "476px";
	document.getElementById("plate1").style.position = "fixed";
	document.getElementById("plate1").style.left = "339px";
	document.getElementById("plate1").style.top = "150px";

	document.getElementById("platefragment").style.backgroundImage = "url('images/plates/platefragment2.png')";
	document.getElementById("platefoam").style.backgroundImage = "url('images/plates/platefoam2.png')";
	document.getElementById("platemicrobeads").style.backgroundImage = "url('images/plates/platemicrobeads2.png')";
	document.getElementById("platefilaments").style.backgroundImage = "url('images/plates/platefilaments2.png')";
	document.getElementById("platefibre").style.backgroundImage = "url('images/plates/platefibre2.png')";
	document.getElementById("platefilm").style.backgroundImage = "url('images/plates/platefilm2.png')";

	document.getElementById("platefragment").style.width = "564px";
	document.getElementById("platefoam").style.width = "564px";
	document.getElementById("platemicrobeads").style.width = "564px";
	document.getElementById("platefilaments").style.width = "564px";
	document.getElementById("platefibre").style.width = "564px";
	document.getElementById("platefilm").style.width = "564px";

	document.getElementById("platefragment").style.height = "337px";
	document.getElementById("platefoam").style.height = "337px";
	document.getElementById("platemicrobeads").style.height = "337px";
	document.getElementById("platefilaments").style.height = "337px";
	document.getElementById("platefibre").style.height = "337px";
	document.getElementById("platefilm").style.height = "337px";

	document.getElementById("fragmentone").style.backgroundImage = "url('images/plates/pet2.png')";
	document.getElementById("fragmentone").style.height = "170px";
	document.getElementById("fragmentone").style.width = "170px";
	document.getElementById("fragmentone").style.left = "989px";
	document.getElementById("fragmentone").style.top = "80px";

	document.getElementById("fragmenttwo").style.backgroundImage = "url('images/plates/ldpe2.png')";
	document.getElementById("fragmenttwo").style.height = "170px";
	document.getElementById("fragmenttwo").style.width = "170px";
	document.getElementById("fragmenttwo").style.left = "604px";
	document.getElementById("fragmenttwo").style.top = "-43px";

	document.getElementById("fragmentthree").style.backgroundImage = "url('images/plates/hdpe2.png')";
	document.getElementById("fragmentthree").style.height = "170px";
	document.getElementById("fragmentthree").style.width = "170px";
	document.getElementById("fragmentthree").style.left = "308px";
	document.getElementById("fragmentthree").style.top = "402px";

	document.getElementById("fragmentfour").style.backgroundImage = "url('images/plates/pvc2.png')";
	document.getElementById("fragmentfour").style.height = "170px";
	document.getElementById("fragmentfour").style.width = "170px";
	document.getElementById("fragmentfour").style.left = "267px";
	document.getElementById("fragmentfour").style.top = "80px";

	document.getElementById("fragmentfive").style.backgroundImage = "url('images/plates/pp2.png')";
	document.getElementById("fragmentfive").style.height = "170px";
	document.getElementById("fragmentfive").style.width = "170px";
	document.getElementById("fragmentfive").style.left = "959px";
	document.getElementById("fragmentfive").style.top = "405px";

	document.getElementById("fragmentsix").style.backgroundImage = "url('images/plates/polystyrene2.png')";
	document.getElementById("fragmentsix").style.height = "170px";
	document.getElementById("fragmentsix").style.width = "170px";
	document.getElementById("fragmentsix").style.left = "611px";
	document.getElementById("fragmentsix").style.top = "509px";

	document.getElementById("foamone").style.backgroundImage = "url('images/plates/polystyrene2.png')";
	document.getElementById("foamone").style.height = "170px";
	document.getElementById("foamone").style.width = "170px";
	document.getElementById("foamone").style.left = "179px";
	document.getElementById("foamone").style.top = "238px";

	document.getElementById("foamtwo").style.backgroundImage = "url('images/plates/ldpehdpe2.png')";
	document.getElementById("foamtwo").style.height = "170px";
	document.getElementById("foamtwo").style.width = "170px";
	document.getElementById("foamtwo").style.left = "1050px";
	document.getElementById("foamtwo").style.top = "238px";

	document.getElementById("microbeadsone").style.backgroundImage = "url('images/plates/nylon2.png')";
	document.getElementById("microbeadsone").style.height = "170px";
	document.getElementById("microbeadsone").style.width = "170px";
	document.getElementById("microbeadsone").style.left = "989px";
	document.getElementById("microbeadsone").style.top = "80px";

	document.getElementById("microbeadstwo").style.backgroundImage = "url('images/plates/ldpehdpe2.png')";
	document.getElementById("microbeadstwo").style.height = "170px";
	document.getElementById("microbeadstwo").style.width = "170px";
	document.getElementById("microbeadstwo").style.left = "604px";
	document.getElementById("microbeadstwo").style.top = "-43px";

	document.getElementById("microbeadsthree").style.backgroundImage = "url('images/plates/pet2.png')";
	document.getElementById("microbeadsthree").style.height = "170px";
	document.getElementById("microbeadsthree").style.width = "170px";
	document.getElementById("microbeadsthree").style.left = "308px";
	document.getElementById("microbeadsthree").style.top = "402px";

	document.getElementById("microbeadsfour").style.backgroundImage = "url('images/plates/phtalate2.png')";
	document.getElementById("microbeadsfour").style.height = "170px";
	document.getElementById("microbeadsfour").style.width = "170px";
	document.getElementById("microbeadsfour").style.left = "267px";
	document.getElementById("microbeadsfour").style.top = "80px";

	document.getElementById("microbeadsfive").style.backgroundImage = "url('images/plates/pp2.png')";
	document.getElementById("microbeadsfive").style.height = "170px";
	document.getElementById("microbeadsfive").style.width = "170px";
	document.getElementById("microbeadsfive").style.left = "959px";
	document.getElementById("microbeadsfive").style.top = "405px";

	document.getElementById("microbeadssix").style.backgroundImage = "url('images/plates/polystyrene2.png')";
	document.getElementById("microbeadssix").style.height = "170px";
	document.getElementById("microbeadssix").style.width = "170px";
	document.getElementById("microbeadssix").style.left = "611px";
	document.getElementById("microbeadssix").style.top = "509px";


	document.getElementById("filamentsone").style.backgroundImage = "url('images/plates/pp2.png')";
	document.getElementById("filamentsone").style.height = "170px";
	document.getElementById("filamentsone").style.width = "170px";
	document.getElementById("filamentsone").style.left = "956px";
	document.getElementById("filamentsone").style.top = "405px";

	document.getElementById("filamentstwo").style.backgroundImage = "url('images/plates/dyes2.png')";
	document.getElementById("filamentstwo").style.height = "170px";
	document.getElementById("filamentstwo").style.width = "170px";
	document.getElementById("filamentstwo").style.left = "620px";
	document.getElementById("filamentstwo").style.top = "-41px";

	document.getElementById("filamentsthree").style.backgroundImage = "url('images/plates/pet2.png')";
	document.getElementById("filamentsthree").style.height = "170px";
	document.getElementById("filamentsthree").style.width = "170px";
	document.getElementById("filamentsthree").style.left = "297px";
	document.getElementById("filamentsthree").style.top = "405px";


	document.getElementById("fibreone").style.backgroundImage = "url('images/plates/phtalate2.png')";
	document.getElementById("fibreone").style.height = "170px";
	document.getElementById("fibreone").style.width = "170px";
	document.getElementById("fibreone").style.left = "1042px";
	document.getElementById("fibreone").style.top = "203px";

	document.getElementById("fibretwo").style.backgroundImage = "url('images/plates/dyes2.png')";
	document.getElementById("fibretwo").style.height = "170px";
	document.getElementById("fibretwo").style.width = "170px";
	document.getElementById("fibretwo").style.left = "616px";
	document.getElementById("fibretwo").style.top = "-45px";

	document.getElementById("fibrethree").style.backgroundImage = "url('images/plates/metals2.png')";
	document.getElementById("fibrethree").style.height = "170px";
	document.getElementById("fibrethree").style.width = "170px";
	document.getElementById("fibrethree").style.left = "458px";
	document.getElementById("fibrethree").style.top = "490px";

	document.getElementById("fibrefour").style.backgroundImage = "url('images/plates/pfc2.png')";
	document.getElementById("fibrefour").style.height = "170px";
	document.getElementById("fibrefour").style.width = "170px";
	document.getElementById("fibrefour").style.left = "229px";
	document.getElementById("fibrefour").style.top = "203px";

	document.getElementById("fibrefive").style.backgroundImage = "url('images/plates/solvents2.png')";
	document.getElementById("fibrefive").style.height = "170px";
	document.getElementById("fibrefive").style.width = "170px";
	document.getElementById("fibrefive").style.left = "774px";
	document.getElementById("fibrefive").style.top = "490px";


	document.getElementById("filmone").style.backgroundImage = "url('images/plates/phtalate2.png')";
	document.getElementById("filmone").style.height = "170px";
	document.getElementById("filmone").style.width = "170px";
	document.getElementById("filmone").style.left = "1042px";
	document.getElementById("filmone").style.top = "203px";

	document.getElementById("filmtwo").style.backgroundImage = "url('images/plates/ldpe2.png')";
	document.getElementById("filmtwo").style.height = "170px";
	document.getElementById("filmtwo").style.width = "170px";
	document.getElementById("filmtwo").style.left = "616px";
	document.getElementById("filmtwo").style.top = "-45px";

	document.getElementById("filmthree").style.backgroundImage = "url('images/plates/hdpe2.png')";
	document.getElementById("filmthree").style.height = "170px";
	document.getElementById("filmthree").style.width = "170px";
	document.getElementById("filmthree").style.left = "640px";
	document.getElementById("filmthree").style.top = "490px";

	document.getElementById("filmfour").style.backgroundImage = "url('images/plates/pvc2.png')";
	document.getElementById("filmfour").style.height = "170px";
	document.getElementById("filmfour").style.width = "170px";
	// document.getElementById("filmfour").style.left = "215px";
	document.getElementById("filmfour").style.top = "203px";


	document.getElementById("fragmentpetone").style.backgroundImage = "url('images/plates/pet2/pet1.png')";
	document.getElementById("fragmentpetone").style.height = "200px";
	document.getElementById("fragmentpetone").style.width = "200px";
	document.getElementById("fragmentpetone").style.left = "294px";
	document.getElementById("fragmentpetone").style.top = "230px";

	document.getElementById("fragmentpettwo").style.backgroundImage = "url('images/plates/pet2/pet2.png')";
	document.getElementById("fragmentpettwo").style.height = "200px";
	document.getElementById("fragmentpettwo").style.width = "200px";
	document.getElementById("fragmentpettwo").style.left = "909px";
	document.getElementById("fragmentpettwo").style.top = "230px";

	document.getElementById("fragmentpetthree").style.backgroundImage = "url('images/plates/pet2/pet3.png')";
	document.getElementById("fragmentpetthree").style.height = "200px";
	document.getElementById("fragmentpetthree").style.width = "200px";
	document.getElementById("fragmentpetthree").style.left = "603px";
	document.getElementById("fragmentpetthree").style.top = "459px";

	document.getElementById("fragmentpetfour").style.backgroundImage = "url('images/plates/pet2/pet4.png')";
	document.getElementById("fragmentpetfour").style.height = "200px";
	document.getElementById("fragmentpetfour").style.width = "200px";
	document.getElementById("fragmentpetfour").style.left = "603px";
	document.getElementById("fragmentpetfour").style.top = "10px";

	document.getElementById("fragmentonedum").style.backgroundImage = "url('images/plates/pet2.png')";
	document.getElementById("fragmentonedum").style.height = "220px";
	document.getElementById("fragmentonedum").style.width = "220px";
	document.getElementById("fragmentonedum").style.top = "235px";
	document.getElementById("fragmentonedum").style.left = "593px";

	document.getElementById("fragmenttwodum").style.backgroundImage = "url('images/plates/ldpe2.png')";
	document.getElementById("fragmenttwodum").style.height = "200px";
	document.getElementById("fragmenttwodum").style.width = "200px";
	document.getElementById("fragmenttwodum").style.left = "605px";
	document.getElementById("fragmenttwodum").style.top = "245px";

	document.getElementById("fragmentldpeone").style.backgroundImage = "url('images/plates/ldpe2/ldpe.png')";
	document.getElementById("fragmentldpeone").style.height = "200px";
	document.getElementById("fragmentldpeone").style.width = "200px";
	document.getElementById("fragmentldpeone").style.left = "603px";
	document.getElementById("fragmentldpeone").style.top = "10px";



	document.getElementById("fragmentthreedum").style.backgroundImage = "url('images/plates/pvc2.png')";
	document.getElementById("fragmentthreedum").style.height = "220px";
	document.getElementById("fragmentthreedum").style.width = "220px";

	document.getElementById("fragmentpvcone").style.backgroundImage = "url('images/plates/pvc2/pvc1.png')";
	document.getElementById("fragmentpvcone").style.height = "200px";
	document.getElementById("fragmentpvcone").style.width = "200px";

	document.getElementById("fragmentpvctwo").style.backgroundImage = "url('images/plates/pvc2/pvc2.png')";
	document.getElementById("fragmentpvctwo").style.height = "200px";
	document.getElementById("fragmentpvctwo").style.width = "200px";

	document.getElementById("fragmentpvcthree").style.backgroundImage = "url('images/plates/pvc2/pvc3.png')";
	document.getElementById("fragmentpvcthree").style.height = "200px";
	document.getElementById("fragmentpvcthree").style.width = "200px";

	document.getElementById("fragmentpvcfour").style.backgroundImage = "url('images/plates/pvc2/pvc4.png')";
	document.getElementById("fragmentpvcfour").style.height = "200px";
	document.getElementById("fragmentpvcfour").style.width = "200px";

	document.getElementById("fragmentpvcfive").style.backgroundImage = "url('images/plates/pvc2/pvc5.png')";
	document.getElementById("fragmentpvcfive").style.height = "200px";
	document.getElementById("fragmentpvcfive").style.width = "200px";

	document.getElementById("fragmentpvcsix").style.backgroundImage = "url('images/plates/pvc2/pvc6.png')";
	document.getElementById("fragmentpvcsix").style.height = "200px";
	document.getElementById("fragmentpvcsix").style.width = "200px";

	document.getElementById("fragmentpvcseven").style.backgroundImage = "url('images/plates/pvc2/pvc7.png')";
	document.getElementById("fragmentpvcseven").style.height = "200px";
	document.getElementById("fragmentpvcseven").style.width = "200px";

	document.getElementById("fragmentpvceight").style.backgroundImage = "url('images/plates/pvc2/pvc8.png')";
	document.getElementById("fragmentpvceight").style.height = "200px";
	document.getElementById("fragmentpvceight").style.width = "200px";


	document.getElementById("fragmentfourdum").style.backgroundImage = "url('images/plates/hdpe2.png')";
	document.getElementById("fragmentfourdum").style.height = "220px";
	document.getElementById("fragmentfourdum").style.width = "220px";

	document.getElementById("fragmenthdpeone").style.backgroundImage = "url('images/plates/hdpe2/hdpe1.png')";
	document.getElementById("fragmenthdpeone").style.height = "200px";
	document.getElementById("fragmenthdpeone").style.width = "200px";

	document.getElementById("fragmenthdpetwo").style.backgroundImage = "url('images/plates/hdpe2/hdpe2.png')";
	document.getElementById("fragmenthdpetwo").style.height = "200px";
	document.getElementById("fragmenthdpetwo").style.width = "200px";

	document.getElementById("fragmenthdpethree").style.backgroundImage = "url('images/plates/hdpe2/hdpe3.png')";
	document.getElementById("fragmenthdpethree").style.height = "200px";
	document.getElementById("fragmenthdpethree").style.width = "200px";

	document.getElementById("fragmenthdpefour").style.backgroundImage = "url('images/plates/hdpe2/hdpe4.png')";
	document.getElementById("fragmenthdpefour").style.height = "200px";
	document.getElementById("fragmenthdpefour").style.width = "200px";

	document.getElementById("fragmenthdpefive").style.backgroundImage = "url('images/plates/hdpe2/hdpe5.png')";
	document.getElementById("fragmenthdpefive").style.height = "200px";
	document.getElementById("fragmenthdpefive").style.width = "200px";

	document.getElementById("fragmenthdpesix").style.backgroundImage = "url('images/plates/hdpe2/hdpe6.png')";
	document.getElementById("fragmenthdpesix").style.height = "200px";
	document.getElementById("fragmenthdpesix").style.width = "200px";

	document.getElementById("fragmentfivedum").style.backgroundImage = "url('images/plates/pp2.png')";
	document.getElementById("fragmentfivedum").style.height = "220px";
	document.getElementById("fragmentfivedum").style.width = "220px";

	document.getElementById("fragmentppone").style.backgroundImage = "url('images/plates/pp2/pp.png')";
	document.getElementById("fragmentppone").style.height = "200px";
	document.getElementById("fragmentppone").style.width = "200px";

	document.getElementById("fragmentsixdum").style.backgroundImage = "url('images/plates/polystyrene2.png')";
	document.getElementById("fragmentsixdum").style.height = "220px";
	document.getElementById("fragmentsixdum").style.width = "220px";

	document.getElementById("foamonedum").style.backgroundImage = "url('images/plates/polystyrene2.png')";
	document.getElementById("foamonedum").style.height = "220px";
	document.getElementById("foamonedum").style.width = "220px";

	document.getElementById("foampolystyreneone").style.backgroundImage = "url('images/plates/polystyrene2/polystyrene.png')";
	document.getElementById("foampolystyreneone").style.height = "200px";
	document.getElementById("foampolystyreneone").style.width = "200px";
	document.getElementById("foampolystyreneone").style.left = "610px";


	document.getElementById("foamtwodum").style.backgroundImage = "url('images/plates/ldpehdpe2.png')";
	document.getElementById("foamtwodum").style.height = "220px";
	document.getElementById("foamtwodum").style.width = "220px";

	document.getElementById("foamhdpeldpeone").style.backgroundImage = "url('images/plates/hdpe2/hdpe1.png')";
	document.getElementById("foamhdpeldpeone").style.height = "200px";
	document.getElementById("foamhdpeldpeone").style.width = "200px";

	document.getElementById("foamhdpeldpetwo").style.backgroundImage = "url('images/plates/hdpe2/hdpe2.png')";
	document.getElementById("foamhdpeldpetwo").style.height = "200px";
	document.getElementById("foamhdpeldpetwo").style.width = "200px";

	document.getElementById("foamhdpeldpethree").style.backgroundImage = "url('images/plates/hdpe2/hdpe3.png')";
	document.getElementById("foamhdpeldpethree").style.height = "200px";
	document.getElementById("foamhdpeldpethree").style.width = "200px";

	document.getElementById("foamhdpeldpefour").style.backgroundImage = "url('images/plates/hdpe2/hdpe4.png')";
	document.getElementById("foamhdpeldpefour").style.height = "200px";
	document.getElementById("foamhdpeldpefour").style.width = "200px";

	document.getElementById("foamhdpeldpefive").style.backgroundImage = "url('images/plates/hdpe2/hdpe5.png')";
	document.getElementById("foamhdpeldpefive").style.height = "200px";
	document.getElementById("foamhdpeldpefive").style.width = "200px";

	document.getElementById("foamhdpeldpesix").style.backgroundImage = "url('images/plates/hdpe2/hdpe6.png')";
	document.getElementById("foamhdpeldpesix").style.height = "200px";
	document.getElementById("foamhdpeldpesix").style.width = "200px";

	document.getElementById("foamhdpeldpeseven").style.backgroundImage = "url('images/plates/ldpe2/ldpe.png')";
	document.getElementById("foamhdpeldpeseven").style.height = "200px";
	document.getElementById("foamhdpeldpeseven").style.width = "200px";


	document.getElementById("microbeadstwodum").style.backgroundImage = "url('images/plates/ldpehdpe2.png')";
	document.getElementById("microbeadstwodum").style.height = "220px";
	document.getElementById("microbeadstwodum").style.width = "220px";

	document.getElementById("microbeadshdpeldpeone").style.backgroundImage = "url('images/plates/hdpe2/hdpe1.png')";
	document.getElementById("microbeadshdpeldpeone").style.height = "200px";
	document.getElementById("microbeadshdpeldpeone").style.width = "200px";

	document.getElementById("microbeadshdpeldpetwo").style.backgroundImage = "url('images/plates/hdpe2/hdpe2.png')";
	document.getElementById("microbeadshdpeldpetwo").style.height = "200px";
	document.getElementById("microbeadshdpeldpetwo").style.width = "200px";

	document.getElementById("microbeadshdpeldpethree").style.backgroundImage = "url('images/plates/hdpe2/hdpe3.png')";
	document.getElementById("microbeadshdpeldpethree").style.height = "200px";
	document.getElementById("microbeadshdpeldpethree").style.width = "200px";

	document.getElementById("microbeadshdpeldpefour").style.backgroundImage = "url('images/plates/hdpe2/hdpe4.png')";
	document.getElementById("microbeadshdpeldpefour").style.height = "200px";
	document.getElementById("microbeadshdpeldpefour").style.width = "200px";

	document.getElementById("microbeadshdpeldpefive").style.backgroundImage = "url('images/plates/hdpe2/hdpe5.png')";
	document.getElementById("microbeadshdpeldpefive").style.height = "200px";
	document.getElementById("microbeadshdpeldpefive").style.width = "200px";

	document.getElementById("microbeadshdpeldpesix").style.backgroundImage = "url('images/plates/hdpe2/hdpe6.png')";
	document.getElementById("microbeadshdpeldpesix").style.height = "200px";
	document.getElementById("microbeadshdpeldpesix").style.width = "200px";

	document.getElementById("microbeadshdpeldpeseven").style.backgroundImage = "url('images/plates/ldpe2/ldpe.png')";
	document.getElementById("microbeadshdpeldpeseven").style.height = "200px";
	document.getElementById("microbeadshdpeldpeseven").style.width = "200px";


	document.getElementById("microbeadsfivedum").style.backgroundImage = "url('images/plates/pp2.png')";
	document.getElementById("microbeadsfivedum").style.height = "220px";
	document.getElementById("microbeadsfivedum").style.width = "220px";

	document.getElementById("microbeadsppone").style.backgroundImage = "url('images/plates/pp2/pp.png')";
	document.getElementById("microbeadsppone").style.height = "200px";
	document.getElementById("microbeadsppone").style.width = "200px";


	document.getElementById("microbeadssixdum").style.backgroundImage = "url('images/plates/polystyrene2.png')";
	document.getElementById("microbeadssixdum").style.height = "220px";
	document.getElementById("microbeadssixdum").style.width = "220px";

	document.getElementById("microbeadspolystyreneone").style.backgroundImage = "url('images/plates/polystyrene2/polystyrene.png')";
	document.getElementById("microbeadspolystyreneone").style.height = "200px";
	document.getElementById("microbeadspolystyreneone").style.width = "200px";
	document.getElementById("microbeadspolystyreneone").style.left = "610px";

	document.getElementById("microbeadsthreedum").style.backgroundImage = "url('images/plates/pet2.png')";
	document.getElementById("microbeadsthreedum").style.height = "220px";
	document.getElementById("microbeadsthreedum").style.width = "220px";
	document.getElementById("microbeadsthreedum").style.top = "235px";
	document.getElementById("microbeadsthreedum").style.left = "593px";

	document.getElementById("microbeadspetone").style.backgroundImage = "url('images/plates/pet2/pet1.png')";
	document.getElementById("microbeadspetone").style.height = "200px";
	document.getElementById("microbeadspetone").style.width = "200px";
	document.getElementById("microbeadspetone").style.left = "294px";
	document.getElementById("microbeadspetone").style.top = "230px";

	document.getElementById("microbeadspettwo").style.backgroundImage = "url('images/plates/pet2/pet2.png')";
	document.getElementById("microbeadspettwo").style.height = "200px";
	document.getElementById("microbeadspettwo").style.width = "200px";
	document.getElementById("microbeadspettwo").style.left = "909px";
	document.getElementById("microbeadspettwo").style.top = "230px";

	document.getElementById("microbeadspetthree").style.backgroundImage = "url('images/plates/pet2/pet3.png')";
	document.getElementById("microbeadspetthree").style.height = "200px";
	document.getElementById("microbeadspetthree").style.width = "200px";
	document.getElementById("microbeadspetthree").style.left = "603px";
	document.getElementById("microbeadspetthree").style.top = "459px";

	document.getElementById("microbeadspetfour").style.backgroundImage = "url('images/plates/pet2/pet4.png')";
	document.getElementById("microbeadspetfour").style.height = "200px";
	document.getElementById("microbeadspetfour").style.width = "200px";
	document.getElementById("microbeadspetfour").style.left = "603px";
	document.getElementById("microbeadspetfour").style.top = "10px";


	document.getElementById("microbeadsfourdum").style.backgroundImage = "url('images/plates/phtalate2.png')";
	document.getElementById("microbeadsfourdum").style.height = "220px";
	document.getElementById("microbeadsfourdum").style.width = "220px";

	document.getElementById("microbeadsphtalateone").style.backgroundImage = "url('images/plates/phtalate2/phtalate1.png')";
	document.getElementById("microbeadsphtalateone").style.height = "200px";
	document.getElementById("microbeadsphtalateone").style.width = "200px";

	document.getElementById("microbeadsphtalatetwo").style.backgroundImage = "url('images/plates/phtalate2/phtalate2.png')";
	document.getElementById("microbeadsphtalatetwo").style.height = "200px";
	document.getElementById("microbeadsphtalatetwo").style.width = "200px";

	document.getElementById("microbeadsphtalatethree").style.backgroundImage = "url('images/plates/phtalate2/phtalate3.png')";
	document.getElementById("microbeadsphtalatethree").style.height = "200px";
	document.getElementById("microbeadsphtalatethree").style.width = "200px";

	document.getElementById("filamentstwodum").style.backgroundImage = "url('images/plates/dyes2.png')";
	document.getElementById("filamentstwodum").style.height = "220px";
	document.getElementById("filamentstwodum").style.width = "220px";

	document.getElementById("filamentsdyesone").style.backgroundImage = "url('images/plates/dyes2/dyes1.png')";
	document.getElementById("filamentsdyesone").style.height = "200px";
	document.getElementById("filamentsdyesone").style.width = "200px";

	document.getElementById("filamentsdyestwo").style.backgroundImage = "url('images/plates/dyes2/dyes2.png')";
	document.getElementById("filamentsdyestwo").style.height = "200px";
	document.getElementById("filamentsdyestwo").style.width = "200px";

	document.getElementById("filamentsdyesthree").style.backgroundImage = "url('images/plates/dyes2/dyes3.png')";
	document.getElementById("filamentsdyesthree").style.height = "200px";
	document.getElementById("filamentsdyesthree").style.width = "200px";


	document.getElementById("filamentspetone").style.backgroundImage = "url('images/plates/pet2/pet1.png')";
	document.getElementById("filamentspetone").style.height = "200px";
	document.getElementById("filamentspetone").style.width = "200px";

	document.getElementById("filamentspettwo").style.backgroundImage = "url('images/plates/pet2/pet2.png')";
	document.getElementById("filamentspettwo").style.height = "200px";
	document.getElementById("filamentspettwo").style.width = "200px";

	document.getElementById("filamentspetthree").style.backgroundImage = "url('images/plates/pet2/pet3.png')";
	document.getElementById("filamentspetthree").style.height = "200px";
	document.getElementById("filamentspetthree").style.width = "200px";

	document.getElementById("filamentspetfour").style.backgroundImage = "url('images/plates/pet2/pet4.png')";
	document.getElementById("filamentspetfour").style.height = "200px";
	document.getElementById("filamentspetfour").style.width = "200px";

	document.getElementById("filamentsonedum").style.backgroundImage = "url('images/plates/pet2.png')";
	document.getElementById("filamentsonedum").style.height = "220px";
	document.getElementById("filamentsonedum").style.width = "220px";


	document.getElementById("filamentsthreedum").style.backgroundImage = "url('images/plates/pp2.png')";
	document.getElementById("filamentsthreedum").style.height = "220px";
	document.getElementById("filamentsthreedum").style.width = "220px";

	document.getElementById("filamentsppone").style.backgroundImage = "url('images/plates/pp2/pp.png')";
	document.getElementById("filamentsppone").style.height = "200px";
	document.getElementById("filamentsppone").style.width = "200px";


	document.getElementById("fibretwodum").style.backgroundImage = "url('images/plates/dyes2.png')";
	document.getElementById("fibretwodum").style.height = "220px";
	document.getElementById("fibretwodum").style.width = "220px";

	document.getElementById("fibredyesone").style.backgroundImage = "url('images/plates/dyes2/dyes1.png')";
	document.getElementById("fibredyesone").style.height = "200px";
	document.getElementById("fibredyesone").style.width = "200px";

	document.getElementById("fibredyestwo").style.backgroundImage = "url('images/plates/dyes2/dyes2.png')";
	document.getElementById("fibredyestwo").style.height = "200px";
	document.getElementById("fibredyestwo").style.width = "200px";

	document.getElementById("fibredyesthree").style.backgroundImage = "url('images/plates/dyes2/dyes3.png')";
	document.getElementById("fibredyesthree").style.height = "200px";
	document.getElementById("fibredyesthree").style.width = "200px";



	document.getElementById("fibrepfcone").style.backgroundImage = "url('images/plates/pfc2/pfc1.png')";
	document.getElementById("fibrepfcone").style.height = "200px";
	document.getElementById("fibrepfcone").style.width = "200px";

	document.getElementById("fibrepfctwo").style.backgroundImage = "url('images/plates/pfc2/pfc2.png')";
	document.getElementById("fibrepfctwo").style.height = "200px";
	document.getElementById("fibrepfctwo").style.width = "200px";

	document.getElementById("fibrepfcthree").style.backgroundImage = "url('images/plates/pfc2/pfc3.png')";
	document.getElementById("fibrepfcthree").style.height = "200px";
	document.getElementById("fibrepfcthree").style.width = "200px";

	document.getElementById("fibrepfcfour").style.backgroundImage = "url('images/plates/pfc2/pfc4.png')";
	document.getElementById("fibrepfcfour").style.height = "200px";
	document.getElementById("fibrepfcfour").style.width = "200px";

	document.getElementById("fibrefourdum").style.backgroundImage = "url('images/plates/pfc2.png')";
	document.getElementById("fibrefourdum").style.height = "220px";
	document.getElementById("fibrefourdum").style.width = "220px";


	document.getElementById("fibreonedum").style.backgroundImage = "url('images/plates/phtalate2.png')";
	document.getElementById("fibreonedum").style.height = "220px";
	document.getElementById("fibreonedum").style.width = "220px";

	document.getElementById("fibrephtalateone").style.backgroundImage = "url('images/plates/phtalate2/phtalate1.png')";
	document.getElementById("fibrephtalateone").style.height = "200px";
	document.getElementById("fibrephtalateone").style.width = "200px";

	document.getElementById("fibrephtalatetwo").style.backgroundImage = "url('images/plates/phtalate2/phtalate2.png')";
	document.getElementById("fibrephtalatetwo").style.height = "200px";
	document.getElementById("fibrephtalatetwo").style.width = "200px";

	document.getElementById("fibrephtalatethree").style.backgroundImage = "url('images/plates/phtalate2/phtalate3.png')";
	document.getElementById("fibrephtalatethree").style.height = "200px";
	document.getElementById("fibrephtalatethree").style.width = "200px";


	document.getElementById("fibrethreedum").style.backgroundImage = "url('images/plates/metals2.png')";
	document.getElementById("fibrethreedum").style.height = "220px";
	document.getElementById("fibrethreedum").style.width = "220px";

	document.getElementById("fibremetalsone").style.backgroundImage = "url('images/plates/metals2/metals1.png')";
	document.getElementById("fibremetalsone").style.height = "200px";
	document.getElementById("fibremetalsone").style.width = "200px";

	document.getElementById("fibremetalstwo").style.backgroundImage = "url('images/plates/metals2/metals2.png')";
	document.getElementById("fibremetalstwo").style.height = "200px";
	document.getElementById("fibremetalstwo").style.width = "200px";

	document.getElementById("fibremetalsthree").style.backgroundImage = "url('images/plates/metals2/metals3.png')";
	document.getElementById("fibremetalsthree").style.height = "200px";
	document.getElementById("fibremetalsthree").style.width = "200px";


	document.getElementById("fibrefivedum").style.backgroundImage = "url('images/plates/solvents2.png')";
	document.getElementById("fibrefivedum").style.height = "220px";
	document.getElementById("fibrefivedum").style.width = "220px";

	document.getElementById("fibresolventone").style.backgroundImage = "url('images/plates/solvents2/solvents1.png')";
	document.getElementById("fibresolventone").style.height = "200px";
	document.getElementById("fibresolventone").style.width = "200px";

	document.getElementById("fibresolventtwo").style.backgroundImage = "url('images/plates/solvents2/solvents2.png')";
	document.getElementById("fibresolventtwo").style.height = "200px";
	document.getElementById("fibresolventtwo").style.width = "200px";


	document.getElementById("filmtwodum").style.backgroundImage = "url('images/plates/ldpe2.png')";
	document.getElementById("filmtwodum").style.height = "200px";
	document.getElementById("filmtwodum").style.width = "200px";

	document.getElementById("filmldpeone").style.backgroundImage = "url('images/plates/ldpe2/ldpe.png')";
	document.getElementById("filmldpeone").style.height = "200px";
	document.getElementById("filmldpeone").style.width = "200px";



	document.getElementById("filmonedum").style.backgroundImage = "url('images/plates/phtalate2.png')";
	document.getElementById("filmonedum").style.height = "220px";
	document.getElementById("filmonedum").style.width = "220px";

	document.getElementById("filmphtalateone").style.backgroundImage = "url('images/plates/phtalate2/phtalate1.png')";
	document.getElementById("filmphtalateone").style.height = "200px";
	document.getElementById("filmphtalateone").style.width = "200px";

	document.getElementById("filmphtalatetwo").style.backgroundImage = "url('images/plates/phtalate2/phtalate2.png')";
	document.getElementById("filmphtalatetwo").style.height = "200px";
	document.getElementById("filmphtalatetwo").style.width = "200px";

	document.getElementById("filmphtalatethree").style.backgroundImage = "url('images/plates/phtalate2/phtalate3.png')";
	document.getElementById("filmphtalatethree").style.height = "200px";
	document.getElementById("filmphtalatethree").style.width = "200px";




	document.getElementById("filmfourdum").style.backgroundImage = "url('images/plates/pvc2.png')";
	document.getElementById("filmfourdum").style.height = "220px";
	document.getElementById("filmfourdum").style.width = "220px";

	document.getElementById("filmpvcone").style.backgroundImage = "url('images/plates/pvc2/pvc1.png')";
	document.getElementById("filmpvcone").style.height = "200px";
	document.getElementById("filmpvcone").style.width = "200px";

	document.getElementById("filmpvctwo").style.backgroundImage = "url('images/plates/pvc2/pvc2.png')";
	document.getElementById("filmpvctwo").style.height = "200px";
	document.getElementById("filmpvctwo").style.width = "200px";

	document.getElementById("filmpvcthree").style.backgroundImage = "url('images/plates/pvc2/pvc3.png')";
	document.getElementById("filmpvcthree").style.height = "200px";
	document.getElementById("filmpvcthree").style.width = "200px";

	document.getElementById("filmpvcfour").style.backgroundImage = "url('images/plates/pvc2/pvc4.png')";
	document.getElementById("filmpvcfour").style.height = "200px";
	document.getElementById("filmpvcfour").style.width = "200px";

	document.getElementById("filmpvcfive").style.backgroundImage = "url('images/plates/pvc2/pvc5.png')";
	document.getElementById("filmpvcfive").style.height = "200px";
	document.getElementById("filmpvcfive").style.width = "200px";

	document.getElementById("filmpvcsix").style.backgroundImage = "url('images/plates/pvc2/pvc6.png')";
	document.getElementById("filmpvcsix").style.height = "200px";
	document.getElementById("filmpvcsix").style.width = "200px";

	document.getElementById("filmpvcseven").style.backgroundImage = "url('images/plates/pvc2/pvc7.png')";
	document.getElementById("filmpvcseven").style.height = "200px";
	document.getElementById("filmpvcseven").style.width = "200px";

	document.getElementById("filmpvceight").style.backgroundImage = "url('images/plates/pvc2/pvc8.png')";
	document.getElementById("filmpvceight").style.height = "200px";
	document.getElementById("filmpvceight").style.width = "200px";



	document.getElementById("filmthreedum").style.backgroundImage = "url('images/plates/hdpe2.png')";
	document.getElementById("filmthreedum").style.height = "220px";
	document.getElementById("filmthreedum").style.width = "220px";

	document.getElementById("filmhdpeone").style.backgroundImage = "url('images/plates/hdpe2/hdpe1.png')";
	document.getElementById("filmhdpeone").style.height = "200px";
	document.getElementById("filmhdpeone").style.width = "200px";

	document.getElementById("filmhdpetwo").style.backgroundImage = "url('images/plates/hdpe2/hdpe2.png')";
	document.getElementById("filmhdpetwo").style.height = "200px";
	document.getElementById("filmhdpetwo").style.width = "200px";

	document.getElementById("filmhdpethree").style.backgroundImage = "url('images/plates/hdpe2/hdpe3.png')";
	document.getElementById("filmhdpethree").style.height = "200px";
	document.getElementById("filmhdpethree").style.width = "200px";

	document.getElementById("filmhdpefour").style.backgroundImage = "url('images/plates/hdpe2/hdpe4.png')";
	document.getElementById("filmhdpefour").style.height = "200px";
	document.getElementById("filmhdpefour").style.width = "200px";

	document.getElementById("filmhdpefive").style.backgroundImage = "url('images/plates/hdpe2/hdpe5.png')";
	document.getElementById("filmhdpefive").style.height = "200px";
	document.getElementById("filmhdpefive").style.width = "200px";

	document.getElementById("filmhdpesix").style.backgroundImage = "url('images/plates/hdpe2/hdpe6.png')";
	document.getElementById("filmhdpesix").style.height = "200px";
	document.getElementById("filmhdpesix").style.width = "200px";
}